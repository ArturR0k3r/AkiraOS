---
layout: default
title: Resources
nav_order: 8
has_children: true
permalink: /resources
---

# Resources

Additional resources for AkiraOS developers.

## Documentation

- [FAQ](faq.md) - Frequently asked questions
- [Glossary](glossary.md) - Technical terminology
- [Performance](performance.md) - Benchmarks and optimization

## External Links

### Official Documentation
- [Zephyr Project](https://docs.zephyrproject.org)
- [WAMR Documentation](https://github.com/bytecodealliance/wasm-micro-runtime/tree/main/doc)
- [WebAssembly Spec](https://webassembly.github.io/spec/)

### Community
- [AkiraOS GitHub](https://github.com/akiraos/AkiraOS)
- [Zephyr Discord](https://chat.zephyrproject.org/)
- [WASM Community](https://webassembly.org/)

### Hardware
- [ESP32-S3 Datasheet](https://www.espressif.com/sites/default/files/documentation/esp32-s3_datasheet_en.pdf)
- [nRF54L15 Product Brief](https://www.nordicsemi.com/Products/nRF54L15)

## Learning Resources

### WebAssembly
- [MDN WebAssembly Guide](https://developer.mozilla.org/en-US/docs/WebAssembly)
- [WASI Tutorial](https://github.com/bytecodealliance/wasmtime/blob/main/docs/WASI-tutorial.md)

### Embedded Development
- [Zephyr Getting Started](https://docs.zephyrproject.org/latest/develop/getting_started/index.html)
- [ESP-IDF Programming Guide](https://docs.espressif.com/projects/esp-idf/)

---

## Tools

- **wasm-tools:** WebAssembly binary toolkit
- **wasm-opt:** WASM optimizer
- **wabt:** WebAssembly Binary Toolkit

---

## Contributing

See [CONTRIBUTING.md](../../CONTRIBUTING.md) for how to contribute to AkiraOS.

---

## License

AkiraOS is licensed under [Apache 2.0](../../LICENSE).
