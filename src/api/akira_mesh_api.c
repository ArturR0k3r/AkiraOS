                            ...
                            uint32_t beacon_interval_ms)
{
    ...
    if (node_id < 0 || node_id > 0xFF) return -EINVAL;
                            ...
                            uint32_t data_ptr, uint32_t data_len)
{
    ...
    if (data_len == 0 || data_len > AKIRA_MESH_API_MAX_PAYLOAD) return -EINVAL;
                                 ...
                                 uint32_t data_len, int32_t max_hops)
{
    ...
    if (data_len == 0 || data_len > AKIRA_MESH_API_MAX_PAYLOAD) return -EINVAL;
    if (max_hops < 0 || max_hops > 0xFF) return -EINVAL;
                                ...
                                uint32_t timeout_ms)
{
    ...
    if (!src_id_out || !buf) return -EFAULT;
