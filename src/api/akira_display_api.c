...
#if defined(CONFIG_AKIRA_OS_SHELL)
#include <console_shell/display_ownership.h>
/**
 * Guard for every WASM-visible draw call: when the OS shell holds the
 * display, WASM apps must not write to the framebuffer or the hardware.
 * Return -EBUSY so the WASM app can retry on the next frame.
 */
#define AKIRA_CHECK_DISPLAY_OWNER_OR_RETURN(ret_val) \
    do { \
        if (akira_display_get_owner() != DISPLAY_OWNER_WASM) { \
            return (ret_val); \
        } \
    } while (0)
#else
