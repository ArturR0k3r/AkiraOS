...
akira_pid_t akira_process_create(const akira_process_options_t *options)
{
    ...
    slot->proc.stack_size = options->stack_size > 0 ? 
                            options->stack_size : AKIRA_PROCESS_DEFAULT_STACK;
    slot->proc.heap_size = options->heap_size > 0 ?
                           options->heap_size : AKIRA_PROCESS_DEFAULT_HEAP;
                       ...
                       akira_duration_t timeout_ms)
{
    ...
    k_timeout_t timeout = (timeout_ms == AKIRA_WAIT_FOREVER) ?
...
int akira_process_set_priority(akira_pid_t pid, akira_process_priority_t priority)
{
    ...
    if (slot->proc.type == AKIRA_PROCESS_NATIVE &&
        ...
        switch (priority) {
        case AKIRA_PROC_PRIORITY_REALTIME: zephyr_priority = -5; break;
        case AKIRA_PROC_PRIORITY_HIGH: zephyr_priority = 5; break;
        case AKIRA_PROC_PRIORITY_NORMAL: zephyr_priority = 10; break;
        case AKIRA_PROC_PRIORITY_LOW: zephyr_priority = 14; break;
        default: zephyr_priority = 15; break;
        }
