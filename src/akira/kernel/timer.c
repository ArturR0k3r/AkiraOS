...
akira_timer_t *akira_timer_create(const akira_timer_config_t *config)
{
    ...
    timer->initial_ms = config->initial_ms > 0 ? 
                        config->initial_ms : config->period_ms;
...
int akira_timer_resume(akira_timer_t *timer)
{
    ...
    k_timeout_t period = (timer->mode == AKIRA_TIMER_ONESHOT) ?
                         K_NO_WAIT : K_MSEC(timer->period_ms);
...
/* Need akira_malloc/akira_free from memory.h */
#include "memory.h"
