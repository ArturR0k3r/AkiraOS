...
struct akira_gpio {
    ...
    uint8_t pin;
    akira_gpio_dir_t dir;
    struct gpio_callback cb;
    akira_gpio_callback_t user_cb;
    void *user_data;
};
...
akira_gpio_t *akira_gpio_open(const akira_gpio_config_t *config)
{
    ...
    akira_gpio_t *gpio = k_malloc(sizeof(akira_gpio_t));
    if (!gpio) {
        return NULL;
    }
...
void akira_gpio_close(akira_gpio_t *gpio)
{
    if (!gpio) return;
...
int akira_gpio_set(akira_gpio_t *gpio, bool value)
{
    if (!gpio) return -1;
    return gpio_pin_set(gpio->port, gpio->pin, value ? 1 : 0);
}

int akira_gpio_get(akira_gpio_t *gpio)
{
    if (!gpio) return -1;
    return gpio_pin_get(gpio->port, gpio->pin);
}

int akira_gpio_toggle(akira_gpio_t *gpio)
{
    if (!gpio) return -1;
...
{
    akira_gpio_t *gpio = CONTAINER_OF(cb, akira_gpio_t, cb);
    if (gpio->user_cb) {
        gpio->user_cb(gpio, gpio->pin, gpio->user_data);
    }
}

int akira_gpio_set_interrupt(akira_gpio_t *gpio, akira_gpio_int_t trigger,
                             akira_gpio_callback_t callback, void *user_data)
{
    if (!gpio) return -1;
...
akira_spi_t *akira_spi_open(const akira_spi_config_t *config)
{
...
void akira_spi_close(akira_spi_t *spi)
{
    if (spi) {
        k_free(spi);
    }
...
int akira_spi_transfer(akira_spi_t *spi, const uint8_t *tx_data,
...
int akira_spi_write(akira_spi_t *spi, const uint8_t *data, size_t len)
{
    return akira_spi_transfer(spi, data, NULL, len);
}

int akira_spi_read(akira_spi_t *spi, uint8_t *data, size_t len)
{
    return akira_spi_transfer(spi, NULL, data, len);
}

int akira_spi_write_read(akira_spi_t *spi,
...
akira_i2c_t *akira_i2c_open(const akira_i2c_config_t *config)
{
    if (!config || !config->bus) {
        return NULL;
    }
    ...
    akira_i2c_t *i2c = k_malloc(sizeof(akira_i2c_t));
    if (!i2c) {
        return NULL;
    }
...
void akira_i2c_close(akira_i2c_t *i2c)
{
    if (i2c) {
        k_free(i2c);
    }
...
int akira_i2c_write(akira_i2c_t *i2c, const uint8_t *data, size_t len)
{
    if (!i2c) return -1;
    return i2c_write(i2c->bus, data, len, i2c->address);
}

int akira_i2c_read(akira_i2c_t *i2c, uint8_t *data, size_t len)
{
    if (!i2c) return -1;
    return i2c_read(i2c->bus, data, len, i2c->address);
}

int akira_i2c_write_read(akira_i2c_t *i2c,
                         const uint8_t *tx_data, size_t tx_len,
                         uint8_t *rx_data, size_t rx_len)
{
    if (!i2c) return -1;
...
int akira_i2c_read_reg(akira_i2c_t *i2c, uint8_t reg,
                       uint8_t *data, size_t len)
{
    return akira_i2c_write_read(i2c, &reg, 1, data, len);
}

int akira_i2c_write_reg(akira_i2c_t *i2c, uint8_t reg,
                        const uint8_t *data, size_t len)
{
    ...
    return akira_i2c_write(i2c, buf, len + 1);
}

int akira_i2c_scan(const char *bus, uint8_t *addresses, size_t max_count)
{
