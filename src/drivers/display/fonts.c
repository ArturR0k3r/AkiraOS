...
void draw_char(int x, int y, char c, uint16_t color, void (*set_pixel)(int, int, uint16_t), FontType font)
{
    ...
    case FONT_HERO:
        if (c < FONT16X28_FIRST_CHAR || c > FONT16X28_LAST_CHAR)
            return;
        {
            int idx = c - FONT16X28_FIRST_CHAR;
            for (int row = 0; row < FONT16X28_HEIGHT; row++)
            {
                uint16_t bits = font_hero[idx][row];
                for (int col = 0; col < FONT16X28_WIDTH; col++)
                {
                    if (bits & (0x8000 >> col))
                    {
                        set_pixel(x + col, y + row, color);
...
void draw_string(int x, int y, const char *str, uint16_t color, void (*set_pixel)(int, int, uint16_t), FontType font)
{
    ...
    int cursor_x = x;
    int cursor_y = y;
    int char_width = (font == FONT_7X10) ? FONT7X10_WIDTH
                     : (font == FONT_HERO) ? FONT16X28_WIDTH : FONT11X18_WIDTH;
    int char_height = (font == FONT_7X10) ? FONT7X10_HEIGHT
                      : (font == FONT_HERO) ? FONT16X28_HEIGHT : FONT11X18_HEIGHT;
