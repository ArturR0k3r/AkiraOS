...
static void draw(int sel) {
    akira_display_clear(C_BLACK); lc_header("DEVELOPER MODE");
    lc_item(0,sel,"Dev Mode",g_devmode_enabled?"ON":"OFF");
    /* Info panel */
    int iy=LIST_Y+ITEM_H+6;
    if (g_devmode_enabled) {
        char buf[80];
        snprintf(buf,sizeof(buf),"OS: %s  Zephyr: %d.%d.%d",
            CONFIG_AKIRA_OS_VERSION,
            (int)KERNEL_VERSION_MAJOR,(int)KERNEL_VERSION_MINOR,(int)KERNEL_PATCHLEVEL);
        akira_display_text(ITEM_X+4,iy,buf,C_GRAY);
    } else {
        akira_display_text(ITEM_X+4,iy,"Enable to see debug info",C_DKGRAY);
    }
    lc_footer("A-Toggle  |  B-Back");
    akira_display_flush();
}
