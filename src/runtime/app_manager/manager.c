...
/* App Manager - lifecycle, install and registry
 * Implementation: name-based APIs that use runtime instance IDs internally.
 */

int app_manager_init(void)
{
    return akira_runtime_init();
}
...
int app_manifest_parse(const char *json, size_t json_len, app_manifest_t *out_manifest)
{
    ...
    /* Use our lightweight JSON helpers */
