...
uint64_t akira_capability_str_to_mask(const char *cap)
{
    ...
    if (strcmp(cap, "matter.*") == 0)       return AKIRA_CAP_MATTER;
    if (strcmp(cap, "mqtt") == 0)           return AKIRA_CAP_MQTT;
    if (strcmp(cap, "mqtt.*") == 0)         return AKIRA_CAP_MQTT;
    if (strcmp(cap, "fs.read") == 0)        return AKIRA_CAP_FS_READ;
    if (strcmp(cap, "fs.write") == 0)       return AKIRA_CAP_FS_WRITE;
    if (strcmp(cap, "fs.*") == 0)           return AKIRA_CAP_FS_READ | AKIRA_CAP_FS_WRITE;
    if (strcmp(cap, "rtc.read") == 0)       return AKIRA_CAP_RTC_READ;
    ...
    if (strcmp(cap, "ble.scan") == 0)       return AKIRA_CAP_BLE_SCAN;
    if (strcmp(cap, "ble.spam") == 0)       return AKIRA_CAP_BLE_SPAM;
    if (strcmp(cap, "mesh") == 0)           return AKIRA_CAP_MESH;
    if (strcmp(cap, "mesh.*") == 0)         return AKIRA_CAP_MESH;
    if (strcmp(cap, "sync") == 0)           return AKIRA_CAP_SYNC;
    if (strcmp(cap, "sync.*") == 0)         return AKIRA_CAP_SYNC;
    /* Wildcard patterns */
    ...
    if (strcmp(cap, "bt.*") == 0)           return AKIRA_CAP_BLE | AKIRA_CAP_HID |
                                                    AKIRA_CAP_BLE_SCAN | AKIRA_CAP_BLE_SPAM;
...
char* akira_capability_mask_to_str(uint64_t cap)
{
    ...
    if (cap & AKIRA_CAP_MQTT)          return "mqtt";
    if (cap & AKIRA_CAP_BLE_SCAN)      return "ble.scan";
    if (cap & AKIRA_CAP_BLE_SPAM)      return "ble.spam";
    if (cap & AKIRA_CAP_FS_READ)       return "fs.read";
    if (cap & AKIRA_CAP_FS_WRITE)      return "fs.write";
    if (cap & AKIRA_CAP_RTC_READ)      return "rtc.read";
    if (cap & AKIRA_CAP_RTC_WRITE)     return "rtc.write";
    if (cap & AKIRA_CAP_CRYPTO)        return "crypto";
    if (cap & AKIRA_CAP_WIFI_INJECT)   return "wifi.inject";
    if (cap & AKIRA_CAP_MESH)          return "mesh";
    if (cap & AKIRA_CAP_SYNC)          return "sync";
    return "unknown";
}
