/**
 * @file akira_os.c
 * @brief AkiraOS boot sequence (akira_start())
 *
 * Brings up the HAL, HID/USB/BT transports, the HTTP server, the WASM runtime
 * and the app manager, then runs the idle loop. Lives in the akira-os module
 * so any application — the reference firmware or product firmware in its own
 * repository — can boot AkiraOS from its main() with akira_start().
 */
...
LOG_MODULE_REGISTER(akira_os, CONFIG_AKIRA_LOG_LEVEL);
